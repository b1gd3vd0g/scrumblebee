...
const isApiKey = (token) => {
  try {
    const payload = jwt.verify(token, process.env.AUTH_API_SECRET);
    return (
      // ensure equality.
      payload.a === process.env.AUTH_API_A &&
      payload.b === process.env.AUTH_API_B &&
      payload.c === process.env.AUTH_API_C &&
      payload.d === process.env.AUTH_API_D
    );
  } catch (e) {
